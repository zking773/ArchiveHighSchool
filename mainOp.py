...
class AsteroidManager(object):
    ...
    def initialize(self, level):
        ...
        self.currentFieldDeterminants = [[0, 0], [0, 0], [0, 0]]

        breadthBound, depthBound, heightBound = 10, 25, 15
        ...
        while distance < self.field_expanse[2][1]:
            ...
            self.genSuccession("Y", self.field_expanse[self.axis_index_dic["Y"]][1]/2, None)

            distance += self.succession_interval[self.axis_index_dic["Y"]]

        minRefPrimer, maxRefPrimer = self.asteroids[0], self.asteroids[-1]

        self.currentFieldDeterminants = [[0, -1], [0, -1], [0, -1]]
    ...
    def genSuccession(self, axis, distance, direction=None):
        ...
        boundIndex = 0 if direction < 0 else 1

        self.currentFieldDeterminants[axis_index][boundIndex] = len(self.asteroids) - 1
    ...
    def maintainAsteroidField(self, avatarPosition, avatarSpeed, dt):
        ...
        if self.initialized:
            ...
            print len(self.asteroids)
            print self.currentFieldDeterminants

            fieldSize = ((self.asteroids[self.currentFieldDeterminants[0][0]], self.asteroids[self.currentFieldDeterminants[0][1]]),
                         (self.asteroids[self.currentFieldDeterminants[1][0]], self.asteroids[self.currentFieldDeterminants[1][1]]),
                         (self.asteroids[self.currentFieldDeterminants[2][0]], self.asteroids[self.currentFieldDeterminants[2][1]]))
        ...
        for i, axis in enumerate(("X", "Y", "Z")):
            ...
            while abs(startPoint) < abs(bound):
                ...
                self.genSuccession(axis, startPoint, spawnDirection)
